...
DBRedis::DBRedis(void) : m_pContext(NULL) {
...
int DBRedis::command(lua_State* tolua_S, bool skipResult){
    if(NULL == m_pContext){
        return 0;
    }
	size_t length;
	size_t queryNumber = lua_objlen(tolua_S, -1);
...
void DBRedis::appendCommand(lua_State* tolua_S){
    if(NULL == m_pContext){
        return;
    }
	size_t length;
    ...
    redisAppendCommandArgv(m_pContext, queryNumber, m_argv.data(), m_argvlen.data());
}
int DBRedis::getReply(lua_State* tolua_S){
    if(NULL == m_pContext){
        return 0;
    }
	redisReply* reply = NULL;
	redisGetReply(m_pContext, (void**)(&reply)); // reply for last operate
    // 状态检测失败，返回错误
    if( !checkReplyStatus(reply) ){
        // 释放掉当前的reply
        freeReplyObject(reply);
...
//            }
            // 2021.4.11 更改成为数字i+1作为key，reply内容作为value；防止出现数组中间为nil导致数据丢失的情况
            lua_createtable(tolua_S, 0, reply->elements);
            for(size_t i=0; i<reply->elements; ++i){
                size_t key = i + 1;
                lua_pushnumber(tolua_S, (lua_Number)key);           // key
                checkRedisReplyData(tolua_S, reply->element[i]);    // value
				lua_rawset(tolua_S, -3);                            // table[key] = value
            }
            break;
        }

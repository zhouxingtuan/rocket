...
function timer_queue:ctor(m_t, m_cb, m_max)
    -- 初始化
    self.m_t = m_t                      -- 检查的间隔时间，秒
    self.m_cb = m_cb                    -- 移除对象时调用的回掉函数
    self.m_max = m_max or 2147483647    -- 最大缓存的数量
    self.m_q = queue.new()
    self.m_m = {}
end
function timer_queue:setKeepTime(m_t)
    self.m_t = m_t
end
function timer_queue:setQueueMax(m_max)
    self.m_max = m_max
end
function timer_queue:doubleQueueMax()
    self.m_max = self.m_max * 2
end
function timer_queue:isFull()
    return self.m_q:count() >= self.m_max
end

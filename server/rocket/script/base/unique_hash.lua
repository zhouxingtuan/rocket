...
function unique_hash:sort(sortFunc)
    table.sort(self.m_array, sortFunc)
end
function unique_hash:first(number)
    local m_array = self.m_array
    local arr = {}
    for k=1,number do
        local value = m_array[k]
        if value then
...
function unique_hash:last(number)
    local m_array = self.m_array
    local stop = #m_array - number
    local arr = {}
    for k=#m_array,stop,-1 do
        local value = m_array[k]
        if value then
            table.insert(arr, value)
        end

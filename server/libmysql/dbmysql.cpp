...
static std::atomic_flag m_initFlag = ATOMIC_FLAG_INIT;
static pthread_mutex_t m_mutex;

void DBMysql::onInitialize(void){
    if(( !m_initFlag.test_and_set()) ){
        pthread_mutex_init(&m_mutex, NULL);
    }
}

DBMysql::DBMysql(void) : m_pMysql(NULL) {

}
DBMysql::~DBMysql(void){
//	LOG_DEBUG("delete function closeMysql");
    closeMysql();
}
bool DBMysql::isAvailable(void){
...
//        LOG_DEBUG("pingMysql closeMysql");
        closeMysql();     // 释放mysql句柄
        return false;
    }
    return true;
...
//    m_pBuffer->resize( length*2+2 );
    m_buffer.resize( length*2+2 );
    result = mysql_real_escape_string(m_pMysql, m_buffer.data(), ptr, length);
//    result = mysql_real_escape_string(m_pMysql, m_pBuffer->data(), ptr, length);
    if( result > 0 ){
//        lua_pushlstring(tolua_S, m_pBuffer->data(), result);
        lua_pushlstring(tolua_S, m_buffer.data(), result);
        return 1;
    }
    return 0;
}
...
//		LOG_DEBUG("executeQuery closeMysql");
//        closeMysql();	// 上层自己关闭
		//查询失败，不写入任何数据
		return 0;
	}
...
bool DBMysql::executeQueryWithoutReturn(const char* query, unsigned int length){
    MYSQL_RES* result;
    if ( mysql_real_query(m_pMysql, query, length) ) {
//        LOG_DEBUG("executeQueryWithoutReturn closeMysql");
//        closeMysql();	// 上层自己关闭
        return false;
    }
...
// 2021.4.6 MySQL的查询结果全部转换成hash表返回，因为数组如果中间有nil时，会导致后面的数据丢失
void DBMysql::parseResult(lua_State* tolua_S, MYSQL_RES* result){
    ...
    lua_createtable(tolua_S, num_rows, 0);
	...
	while ( (row = mysql_fetch_row( result )) ) {
	    ...
	    lua_createtable(tolua_S, 0, num_fields);
		for(unsigned int i = 0; i < num_fields; i++) {
            //数字全部使用double来表示，其它用string
            if(row[i] != NULL){
                // key
                lua_pushstring(tolua_S, field_names[i].c_str());
                // value
...
void DBMysql::parseOther(lua_State* tolua_S, MYSQL_RES* result){
    static std::string affected_rows("affected_rows");
    static std::string why_come_here("why_come_here");
    unsigned int num_rows;
	//result begin
	if( mysql_errno(m_pMysql) ){
		...
		lua_pushlstring(tolua_S, affected_rows.c_str(), affected_rows.length());
		lua_rawseti(tolua_S, -2, 1);
		...
		lua_createtable(tolua_S, 1, 0);
		lua_pushlstring(tolua_S, why_come_here.c_str(), why_come_here.length());
		lua_rawseti(tolua_S, -2, 1);
	}
}

...
HttpsClient::HttpsClient(void) : HttpClient(), m_pSSL(NULL) {
//    setType(CONN_TYPE_HTTPS_CLIENT);
}
HttpsClient::~HttpsClient(void){
    destroySSL();
}

bool HttpsClient::epollActive(uint32 events){
    // 如果是SSL验证状态
...
int HttpsClient::readSocket(void){
...
int HttpsClient::writeSocket(void){
    int nwrite;
...
bool HttpsClient::bindSSL(SSL_CTX* ctx){
    destroySSL();
	/* 基于 ctx 产生一个新的 SSL*/
    m_pSSL = SSL_new(ctx);
...
void HttpsClient::startConnecting(bool changeEpoll){
...
void HttpsClient::clearData(void){
	destroySSL();
	HttpCommon::clearData();
}
void HttpsClient::destroySSL(void){
	if(NULL != m_pSSL){
		/* 关闭 SSL 连接 */
		SSL_shutdown(m_pSSL);
        /* 释放 SSL*/
